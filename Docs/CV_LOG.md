...
## Урок 02 — Service Locator і перший сервіс

### C# — мова
- Маркерний інтерфейс (`IService`, без методів) як generic-обмеження, що звужує "що
  можна покласти в контейнер", а не описує поведінку `[перевірено]`
- Eager static singleton з приватним конструктором (`public static AllServices Instance
  { get; } = new AllServices();`) — ще один спосіб гарантувати рівно один інстанс, окремо
  від DI-контейнера `[перевірено]`
- `Resources.Load<T>(string)`/`Object.Instantiate(...)` — типізоване завантаження й
  інстанціювання асетів, обгорнуте за інтерфейсом замість прямих викликів по всьому
  коду `[перевірено]`

### Патерни й архітектура
- Service Locator (`AllServices`) — центральний реєстр сервісів за типом-ключем, зі
  свідомим і вузьким винятком із заборони на публічні статичні синглтони `[перевірено]`
- Provider pattern (`IAssetProvider`/`AssetProvider`) — інтерфейс над джерелом асетів
  (`Resources` зараз, Addressables в уроці 15), споживач не знає, звідки фізично
  береться об'єкт `[перевірено]`
- YAGNI на практиці — виявлення й прибирання невикористаних параметрів конструктора
  (`SceneLoader` у `BootstrapState`, `GameStateMachine` у `GameLoopState`), скопійованих
  по аналогії із сусідніх класів, а не тому, що клас реально ними користується
  `[перевірено]`

### Unity
- Читання стек-трейсу Console для перевірки порядку викликів через кілька шарів
  (`RegisterServices() → RegisterService<T>()`, потім окремо `LoadAsset →
  SpawnAsset`) `[перевірено]`

---

## Урок 03 — `IInputService` і новий Input System

### C# — мова
- Лямбда-вирази як анонімні методи (`_ => OnJumpPressed?.Invoke()`), у т.ч. параметр
  `_`/`ctx` для навмисно невикористаного аргументу, компілятором виведений тип
  параметра з типу делегата події `[перевірено]`
- `event Action<T>` — подія з параметром, обов'язковий збіг сигнатури обробника з
  типом делегата (одна параметризована й одна безпараметрова подія в одному проєкті)
  `[перевірено]`
- Вкладені типи (nested types) — `InputAction.CallbackContext` як приклад із
  зовнішньої бібліотеки, синтаксис `Зовнішній.Вкладений` `[вивчено]`
- Різниця іменованого методу й інлайн-лямбди для підписки/відписки — лямбда щоразу
  створює новий делегат-об'єкт, тому `-=` спрацьовує лише з іменованим методом
  `[перевірено]`

### Патерни й архітектура
- Другий сервіс у `AllServices` без жодної правки самого контейнера — практичний
  доказ OCP на реальному прикладі (`IInputService` поруч з `IAssetProvider`)
  `[перевірено]`
- Interface Segregation на `IInputService` — контракт містить лише
  `GetDirection()`/`OnJumpPressed`, а не весь API згенерованого Input System класу
  `[перевірено]`
- Подія-переклад (event relay) — `InputService` одночасно підписник на Unity-подію й
  видавець власної, простішої події; переносить DIP-межу (`UnityEngine.InputSystem`
  ізольований в одному класі) `[перевірено]`
- Симетрична підписка/відписка (`Enter`/`Exit`) для довготривалого підписника,
  застосована вдруге в проєкті (після завіси уроку 01) `[перевірено]`

### Unity
- Новий Input System (`com.unity.inputsystem`): Input Actions asset, Action Map,
  Binding, Composite Binding, `Generate C# Class` `[перевірено]`
- Poll (`ReadValue<T>()`) vs Event (`.started`/`.performed`/`.canceled`) — коли який
  підхід доречний (`Move` — poll, `Jump` — event) `[перевірено]`
- `InputAction`/`CallbackContext` без обгортки (`new InputAction(...)`), `Interaction`
  (`hold`) як приклад, коли `.started`/`.performed`/`.canceled` — справді різні
  моменти, а не один і той самий `[вивчено]`
- Читання Play Mode стек-трейсу через рушій Input System
  (`NativeInputRuntime`/`NativeInputSystem:NotifyUpdate`) як доказ того, хто реально
  викликає `Invoke()` для вбудованих подій `[перевірено]`
