...
**Чому саме "статичний синглтон" (`GameBootstrapper.Instance`) — типовий антипатерн, якого
курс свідомо уникає:**
1. Читаючи конструктор класу, ти вже не бачиш його реальних залежностей — частина з них
   "ховається" всередині методів через `.Instance`.
2. Неможливо підмінити залежність (наприклад, для тесту) — вона жорстко зашита в статику.
3. Залежить від порядку ініціалізації — якщо хтось звернеться до `.Instance` до того, як
   `Awake()` встиг його призначити, впаде `NullReferenceException`.

Composition Root вирішує всі три: залежності видно одразу в сигнатурі конструктора, і
жодного глобального стану немає взагалі.
