...
- [`Kinematics_For_Jump.md`](Kinematics_For_Jump.md) — шкільна кінематика для стрибка, для
  тих, хто забув фізику: розділ 0 (як читати формули, словник символів, за яких умов формули
  працюють), таблиця «м'яч підкинули з 10 м/с», кожна формула з розписаною алгеброю
  (`v = v0 − g·t`, `y = v0·t − g·t²/2`, час підйому, висота стрибка, `v0 = √(2·g·h)`, падіння,
  як результат залежить від `h`), перевірка одиниць, кадри й `dt`, таблиця «формула → рядок
  `PlayerController`», чому покадровий код трохи не збігається з формулою (і залежність від FPS),
  покадровий розбір стрибка, змінні коду, типові помилки, перевірка в грі.
